...
$('#partisan-radio > label.btn').on('click', function () {
    $checked = $('#partisan-radio > label.btn.btn-primary');
...
$('#unionist-radio > label.btn').on('click', function () {
    $checked = $('#unionist-radio > label.btn.btn-primary');
    $inputOld = $('#unionist-radio > label.btn.btn-primary > input');
...
$('#input-file-activity-list').change(handleFileActivityList);

function handleFileActivityList(e) {
    ...
    for (i = 0; i != files.length; ++i) {
        ...
        reader.onload = function (e) {
            ...
            jsonContent.forEach(function(data) {
                htmlContent += '<tr><td>'+ data['test1'] +'</td><td>'+ data['test2'] + '</td></tr>';
            });

            $('#test-xlsx tbody').html(htmlContent);
        };
        reader.readAsBinaryString(f);
    }
}

$('#add-one-student-activity-button').on('click', function() {
    var check = $('#add-attender').is(':visible');
    if(!check) {
        $('#add-attender').fadeIn('slow');
    } else {
        $('#add-attender').fadeOut('slow');
    }
});